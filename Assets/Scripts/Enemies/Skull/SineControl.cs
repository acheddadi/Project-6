...
public class SineControl : MonoBehaviour
{
    ...
    public float spriteFlipDelay = 2.0f, travelDistance = 1.0f, travelDelay = 1.0f, swayDistance = 0.5f, swaySpeed = 2.0f;
    ...
    private void Update()
    {
        ...
        if (cosine)
        {
            ...
            _initialY + swayDistance * Mathf.Cos(_sineTimer * swaySpeed));
        }
        else
        {
            transform.position = new Vector3(
            transform.position.x,
            _initialY + swayDistance * Mathf.Sin(_sineTimer * swaySpeed));
        }
